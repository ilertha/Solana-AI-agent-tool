...
import { TOKENS, DEFAULT_OPTIONS, JUP_API } from "solana-agent-kit/dist/constants";
...
// Extend SolanaAgentKit interface
...
export async function trade(
  ...
  inputAmount: number,
  inputMint: PublicKey = new PublicKey(TOKENS.USDC),
  slippageBps: number = DEFAULT_OPTIONS.SLIPPAGE_BPS,
): Promise<string> {
  try {
    // Check if input token is native SOL
    const isNativeSol = inputMint.equals(new PublicKey(TOKENS.SOL));
    ...
    const quoteResponse = await (
      await fetch(
        `${JUP_API}/quote?` +
          ...
          `&maxAccounts=20` +
          `${agent.options.JUPITER_FEE_BPS ? `&platformFeeBps=${agent.options.JUPITER_FEE_BPS}` : ""}`,
      )
    ).json();

    // Get serialized transaction
    let feeAccount;
    if (agent.options.JUPITER_REFERRAL_ACCOUNT) {
      [feeAccount] = PublicKey.findProgramAddressSync(
        [
          Buffer.from("referral_ata"),
          new PublicKey(agent.options.JUPITER_REFERRAL_ACCOUNT).toBuffer(),
          new PublicKey(TOKENS.SOL).toBuffer(),
        ],
